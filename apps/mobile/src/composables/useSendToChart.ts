...
// Attribution text lives in the site profile: API writes are force-zeroed to
// UserNum 0, so Open Dental shows no author and the line inside the text is
// the only attribution. It names the provider ON THE CASE rather than a fixed
// name — a covering dentist should not be recorded as the practice owner.
          ...
          await postCommlog(
            {
              ...
              note: `${ACTIVE_SITE.noteAttribution(patient.provider)}\n\n${clinicalNoteToText(note.value)}`,
            },
            creds,
          );
